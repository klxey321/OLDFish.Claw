...
- 页面访问可开启用户名/密码登录。
- API 仍保留 `x-local-token` 本地令牌模式，方便 Master 拉 Edge 摘要。
- 登录态使用签名 Cookie，会话时长由 `SESSION_TTL_HOURS` 控制。
- 生产环境必须设置强随机 `SESSION_SECRET`，不要把登录口令直接提交进仓库。

## 当前体验优化

- 菜单切页加入预取与离场过渡，避免切换时完全空白。
- Master 页面摘要改为短时缓存，减少每次切页都重算全量数据。
- 记忆/文档工作台只在对应页面加载，降低普通页面首屏压力。
